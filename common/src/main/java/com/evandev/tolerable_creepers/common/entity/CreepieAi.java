...
public class CreepieAi {
    ...
    private static void updateTarget(LivingEntity owner, Creepie creepie) {
        ...
        if (lastHurt == null) {
            creepie.getBrain().setMemory(MemoryModuleType.NEAREST_ATTACKABLE, lastHurtBy);
            return;
        }

        if (lastHurtBy == null) {
            creepie.getBrain().setMemory(MemoryModuleType.NEAREST_ATTACKABLE, lastHurt);
            return;
        }

        long lastHurtTime = owner.getLastHurtMobTimestamp();
        long lastHurtByTime = owner.getLastHurtByMobTimestamp();
        creepie.getBrain().setMemory(MemoryModuleType.NEAREST_ATTACKABLE, lastHurtByTime >= lastHurtTime ? lastHurtBy : lastHurt);
    }
    ...
    protected static void updateActivity(Creepie creepie) {
        ...
        if (brain.hasMemoryValue(MemoryModuleType.DANCING)) {
            ...
            if (!creepie.isDancing()) {
                creepie.setDancing(true);
            }
        } else if (creepie.isDancing()) {
            creepie.setDancing(false);
        }
